<template>
  <div>
    <el-collapse :value="openNames">
      ...
      <el-collapse-item title="任务内容"
                        name="3">
        <task-item :data="task"></task-item>
      </el-collapse-item>

      <el-collapse-item title="需求分析和需求确认单"
                        name="4"
                        v-if="Object.keys(requestInfo).length>0">
        <request-info :data="requestInfo"></request-info>
      </el-collapse-item>

      <el-collapse-item title="进度列表"
                        name="5"
      ...
      <el-collapse-item title="成果情况"
                        name="5"
                        v-if="Object.keys(finishInfo).length>0">
        <finish-item :data="finishInfo"></finish-item>
      </el-collapse-item>

      <el-collapse-item title="任务审核情况"
                        name="6"
    ...
    <div class="btn-group"
      ...
      <el-button type="primary"
                 plain
                 @click="xqfxVisible=true">提交需求分析和需求确认单</el-button>
      <el-button type="primary"
                 plain
                 @click="processVisible=true">提交进度</el-button>
      <el-button type="warning"
                 plain
                 @click="finishVisible=true">提交审核</el-button>
      <el-button type="primary"
                 plain
                 @click="finishReview">审核</el-button>
    </div>

    <el-drawer title="提交需求分析和需求确认单"
               :visible.sync="xqfxVisible"
               size="450px"
               :wrapperClosable="false">
      <request-add v-if="xqfxVisible"
                   @on-success="init()"></request-add>
    </el-drawer>

    <el-dialog title="提交进度"
               :visible.sync="processVisible"
               width="550px"
               :close-on-click-modal="false">
      <process-add v-if="processVisible"
                   @on-success="init()"></process-add>
    </el-dialog>
    ...
    <el-dialog title="提交审核"
               ...
               width="550px"
               :close-on-click-modal="false">
      <finish-add v-if="finishVisible"
                  @on-success="init()"></finish-add>
    </el-dialog>

    <el-dialog title="审核意见"
               :visible.sync="reviewVisible"
               :close-on-click-modal="false">
      <finish-review v-if="reviewVisible"
                     @on-success="init()"
                     :data="finishReviewData"></finish-review>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  mounted () {
    ...
    this.$flowableClient.tasks
      .getTask(this.$route.query.flowableTaskId)
      .then((task) => {
        this.flowableTask = task.data

        if (this.flowableTask.name === '') {
          //todo 設置按鈕是否可見
        }
      });
  },
  methods: {
    ...
    finishReview () {
      this.finishReviewData = {
        task: this.task,
        flowableTask: this.flowableTask
      }
