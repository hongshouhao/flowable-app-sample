...
function ProcessInstancesResource (options, http) {
  ...
  this.getDiagram = (processInstanceId) => {
    http.config.headers['content-type'] = 'image/png'
    return http.get(resourcePath + `/${processInstanceId}/diagram`,
      getRequestArgs()
    )
  }

  this.getDiagramUrl = (processInstanceId) => {
    return http.defaults.originBaseURL + resourcePath + `/${processInstanceId}/diagram`
  }
